-- Script para corrigir os tipos de timestamp na função get_registration_details

-- Drop existing function first
DROP FUNCTION IF EXISTS get_registration_details(uuid);

-- Create corrected function with proper timestamp types
...
RETURNS TABLE (
  ...
  registration_created_at TIMESTAMPTZ,  -- Corrigido: TIMESTAMPTZ
  attendance_confirmed BOOLEAN,
  attendance_confirmed_at TIMESTAMPTZ,  -- Corrigido: TIMESTAMPTZ
  purchased_glasses BOOLEAN,
  glasses_purchase_amount DECIMAL,
  process_completed BOOLEAN,
  completed_at TIMESTAMPTZ,  -- Corrigido: TIMESTAMPTZ
  attended_by TEXT,
  organizer_name TEXT,
