  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PromoterV2[]> => {
      try {
        ...
        let query = supabase
          .from('organizers')
          .select('id, name, email, phone, city, state, status, role, asaas_api_key, asaas_wallet_id, created_at, updated_at')
    ...
    mutationFn: async (data: PromoterCreation) => {
      try {
        ...
        const { data: promoter, error: dbError } = await supabase
          .from('organizers')
          .insert([insertData])
          .select('id, name, email, phone, city, state, status, role, created_at')
          .single()
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string, data: Partial<PromoterV2> }) => {
      try {
        ...
        // Adicionar asaas_wallet_id apenas se fornecido
        if (data.asaas_wallet_id !== undefined) {
          updateData.asaas_wallet_id = data.asaas_wallet_id || null
        }
